 ...
 * hmauroy.github.io
 * 
 * Added decoding of byte 16 for Mitsubishi MFZ floor models.
 * Byte 16 controls the lower vane. 
 * 
 */
...
bool decodeMitsubishiElectric(byte *bytes, int byteCount)
{
  ...
  if ( byteCount == 36 && bytes[0] == 0x23 &&
    ...
    // Lower vane (MFZ floor model)
    switch (bytes[16]) {
      case 0x00:
        Serial.println(F("LOWER VANE: Open"));
        break;
      case 0x02:
        Serial.println(F("LOWER VANE: Closed"));
        break;
    }
